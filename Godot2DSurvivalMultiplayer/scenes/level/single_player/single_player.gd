extends Node2D
@export var players_container: Node2D # sort pls
@onready var main_menu = $CanvasLayer/SinglePlayerSelect
...
func _start_singleplayer_game() -> void:
	...
	# Local-only world gen, no seed sync needed since there are no peers.
	tilemap._initiate(randi())

	# start_singleplayer() already ran (and already emitted player_connected)
	# before this scene loaded, so we spawn directly from Network's stored data
	# instead of waiting on the signal.
	if Network.players.has(1):
		_add_player(1, Network.players[1])
	else:
		push_error("Singleplayer scene loaded but Network.players[1] wasn't set - did you call Network.start_singleplayer() before changing scenes?")


func _on_upnp_ready(success: bool, external_ip: String) -> void:
	if success:
		print("UPnP succeeded - friends can join using: ", external_ip)
		# TODO: wire this into your UI, e.g. main_menu.set_join_address(external_ip)
